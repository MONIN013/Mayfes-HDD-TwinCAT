...
if isfield(status, "statuswords") && numel(status.statuswords) > 1
    status_text = join(compose("0x%04X", status.statuswords), ",");
    control_text = join(compose("0x%04X", status.controlwords), ",");
    output_text = join(string(status.outputValues), ",");
    fprintf("Statusword=[%s], Controlword=[%s], State=%s (%d), idx=%d/%d, repeat=%d, cycle=%d, output=[%s], error=%d\n", ...
        status_text, control_text, status.stateName, status.state, ...
        status.idx, status.tableLen, status.repeatCount, status.cycleCount, ...
        output_text, status.errorCode);
else
